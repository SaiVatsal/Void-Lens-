"""Instagram — username profile check via profile page probe."""
...
class InstagramUsername(BaseUsernameModule):
    def metadata(self) -> dict:
        return {"name": "Instagram", "url": "https://instagram.com", "category": Category.SOCIAL}
    ...
    async def check(self, username: str, engine) -> SiteResult:
        ...
        try:
            ...
            if status_code == 200 and username.lower() in body.lower():
                return SiteResult(
                    site="Instagram", status=Status.FOUND, url=url,
                    category=Category.SOCIAL, reason="Profile exists",
                    response_time=elapsed, status_code=status_code,
                )
            if status_code == 404:
                return SiteResult(
                    site="Instagram", status=Status.NOT_FOUND, url=url,
                    category=Category.SOCIAL, reason="Not found",
                    response_time=elapsed, status_code=status_code,
                )
            return SiteResult(site="Instagram", status=Status.UNKNOWN, url=url,
                              category=Category.SOCIAL, reason=f"HTTP {status_code}",
                              response_time=elapsed, status_code=status_code)
        except Exception as exc:
            return SiteResult(site="Instagram", status=Status.ERROR, url=url,
